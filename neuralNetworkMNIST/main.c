...
#define LEARNING_RATE 0.001f // learning rate for gradient descent
...
#define MINI_BATCH_SIZE 1 // each mini batch is 100 data sets
#define TESTING_DATA_SETS 10000 // 10,000 mnist test data sets
#define EPOCH 1 //10 // go through the training data set x times

#define OPTIMIZER_TYPE 3 // type: SGD (0), Momentum (1), RMSProp (2), Adam (3)
#define MOMENTUM_COEFFICIENT 0.9f //0.3f // number between 0 and 1. Zero to use no momentum
#define RMS_DECAY_RATE 0.999f //0.5f // the decay rate of the gradients for RMSProp. How much of the previous gradients should we include in the new calculated gradient?

// debug constants (0 for false and 1 for true)
#define LOG_ACCURACY 0

// seed for the random number generator
#define RNG_SEED time(NULL) // usually time(NULL)
...
int main(void) {
    ...
    if (LOG_ACCURACY) {
        printf("Clearing accuracy files\n");

        // train accuracy
        fp = fopen("trainAccuracy.csv", "w");
        if (fp == NULL) {
            perror("Could not open trainAccuracy.csv");
            return 1;
        }
        fclose(fp);

        // test accuracy
        fp = fopen("testAccuracy.csv", "w");
        if (fp == NULL) {
            perror("Could not open testAccuracy.csv");
            return 1;
        }
        fclose(fp);
    }
    ...
    Optimizer *optimizer = initializeOptimizer(network, OPTIMIZER_TYPE, LEARNING_RATE);
    if (optimizer == NULL) {
    ...
    applyOptimizer(network, optimizer);
    ...
    for (int epoch = 0; epoch < EPOCH; epoch++) {
        ...
        int trainAccuracyFileLine = 0;
        int testAccuracyFileLine = 0;

        // open accuracy.csv file
        FILE *fp;
        if (LOG_ACCURACY) {
            fp = fopen("trainAccuracy.csv", "a");
            if (fp == NULL) {
                perror("Error opening file");
                return 1;
            }
        ...
        for (int dataIndex = 0; dataIndex < TRAINING_DATA_SETS; dataIndex++) {
            // feed forward and accumulate the gradients for the batch
            feedForward(network, &processedTrainingInput[dataIndex][0]);

            // accumulate the gradients for the batch
            computeGradients(network, &processedTrainingOutput[dataIndex][0]);
            //backPropagate(network, &processedTrainingOutput[dataIndex][0]);
            //printNetwork(network);

            // calculate cost with MSE
            cost = 0;
            for (int currentOutputNode = 0; currentOutputNode < OUTPUT_NODES; currentOutputNode++) {
                cost += pow(processedTrainingOutput[dataIndex][currentOutputNode] - network->layers[network->layerCount - 1].values[currentOutputNode], 2);
            }
            cost /= OUTPUT_NODES;
            ...
            for (int currentOutputNode = 0; currentOutputNode < OUTPUT_NODES; currentOutputNode++) {
                ...
                if (processedTrainingOutput[dataIndex][currentOutputNode] == 1) {
            ...
            if (dataIndex % MINI_BATCH_SIZE == 0 && dataIndex != 0) {
                // average the accumulated batch gradients, average them & zero the gradients to prepare for next batch
                averageAccumulatedGradients(network, MINI_BATCH_SIZE);
                SGDUpdate(network);
                ...
                if (LOG_ACCURACY) {
                    // write training accuracy to accuracy.csv file
                    fprintf(fp, "%d, %f\n", trainAccuracyFileLine, (totalTrainingAccuracy / dataIndex) * 100);
                    trainAccuracyFileLine++;
                }

                // print info
                printf("Batch #%d completed\n", dataIndex / MINI_BATCH_SIZE);
                printf("Epoch %d/%d, Training Data Set %d\n", epoch+1, EPOCH, dataIndex/MINI_BATCH_SIZE);
                for (int i = 0; i < OUTPUT_NODES; i++) {
                    printf("Output[%d]: %f, Expected: %f\n", i, network->layers[network->layerCount - 1].values[i], processedTrainingOutput[dataIndex][i]);
                }
                printf("Most Likely: %d, Actual Value: %d\n", mostLikely, actualOutput);
                printf("Cost: %f, Current Training Accuracy: %f%%\n", cost, (float)((totalTrainingAccuracy / (TRAINING_DATA_SETS*epoch+dataIndex)) * 100));
                printf("\n");
                displayImage(&processedTrainingInput[dataIndex][0], (int)sqrt(INPUT_NODES), (int)sqrt(INPUT_NODES));
                printf("\n%f%% Finished Epoch %d\n", ((float)dataIndex / TRAINING_DATA_SETS) * 100, epoch+1);
                printf("\n");
            }
...
void *trainOnBatch(void *args) {
    ...
    for (int currentDataSet = 0; currentDataSet < batchSize; currentDataSet++) {
        ...
        // accumulate the gradients for the batch
        computeGradients(network, &batchDataOutput[currentDataSet][0]);
        //backPropagate(network, &batchDataOutput[currentDataSet][0]);
        //printNetwork(network);
    ...
    // average the accumulated batch gradients, average them & zero the gradients to prepare for next batch
    averageAccumulatedGradients(network, batchSize);
    SGDUpdate(network);
    zeroGradients(network);
    ...
    if (LOG_ACCURACY) {
        ...
        // write training accuracy to accuracy.csv file
        fprintf(fp, "%d, %f\n", trainAccuracyFileLine, (totalTrainingAccuracy / (batchSize * totalThreadsCompleted)) * 100);
        trainAccuracyFileLine++;
...
void testNetwork(float *inputData, int inputDataSize, float *outputData, int outputDataSize, int dataLength) {
    ...
    if (LOG_ACCURACY) {
        // open accuracy.csv file
        FILE *fp = fopen("testAccuracy.csv", "a");
        if (fp == NULL) {
            perror("Error opening file");
            return;
        }
        ...
        fprintf(fp, "%d, %f\n", testAccuracyFileLine, (float)testCorrect / TESTING_DATA_SETS * 100);
        testAccuracyFileLine++;
